function [h1,h2]=extraction_all(value,name)
...
rangex=rangex(2:end);
bb0=bb0(2:end);
bb0_temp=(bb0-14.8)./(8.8-14.8);
loca_bb0=find(bb0_temp<1e-2);
...
bench=trapz(rangex(1:loca_bb0(1)),cao.*rangex(1:loca_bb0(1)));

if rem(index,2)==1
h_final=pyrunfile("sol.py","e1",X=[value(1),value(2),value(3),value(4),value(5),value(6),index2]);
...
ub = [];
A0_rev=@(h,r) r.*A0(h,r);
rangey=(bb0-14.8)./(8.8-14.8);
ref=trapz(rangex(1:loca_bb0(1)),(A0(h_final,rangex(1:loca_bb0(1)))-cao).*rangex(1:loca_bb0(1)));
else
...
fun11=@(h,r) quadgk(@(k) -exp(h.*r.^2./(2.*beta.*H.*rho)-h.*k.^2./(2.*beta.*H.*rho)+log(f(k))+log(h.*k./(beta.*H.*rho))),min(best_r,sqrt(r.^2+2.*beta.*t)),min(best_r,r));
T0=@(h,r) real(arrayfun(@(r0) fun10(h,r0)+fun11(h,r0),r));
options = optimoptions('lsqcurvefit','MaxFunctionEvaluations',1e10);
lb = [];
ub = [];
T0_rev=@(h,r) r.*T0(h,r);
rangey=(bb0-14.8)./(8.8-14.8);
h_final=pyrunfile("sol.py","e2",X=[value(1),value(2),value(3),value(4),value(5),value(6),index2]);
SplineFit = fit(x(1:end)',max(T0(h_final,x(1:end))',0), 'smoothingspline');
% ref=trapz(rangex(1:loca_bb0(1)),(feval(SplineFit,rangex(1:loca_bb0(1)))-cao).*rangex(1:loca_bb0(1)));
% loss2(end+1)=abs(ref)./bench;
bb=feval(SplineFit,x(1));
h2(end+1)=h_final;
if rem(index2,5)==1
figure(2);
plot(rangex(1:end),feval(SplineFit,rangex(1:end)),'g--');
hold on;
end
end
end
